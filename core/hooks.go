...
// SetListener registers an in-process listener that receives every emitted
// lifecycle event (used e.g. by the observer plugin to track real replies).
func (hm *HookManager) SetListener(fn func(HookEvent)) {
	hm.mu.Lock()
	hm.listener = fn
	hm.mu.Unlock()
}
